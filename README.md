...
以下三张截图均来自当前源码本地 Release 构建的真实 Windows 窗口（125% DPI），使用临时目录中的公开示例配置；不包含个人启动项。官网静态源码位于 [`docs/`](docs/index.html)，提供分类、搜索与设置的交互模拟。
...
- 顶部加号统一提供“添加项目”和“新建分类”，分类栏保留完整空间。
...
- **GDI / GDI+ / DWM**：界面绘制、过渡、圆角与阴影；系统自带 GDI+ 为开关和八齿齿轮提供抗锯齿
- **Windows 主题**：按窗口 DPI 查询复选框实际尺寸并居中绘制，主题不可用时使用系统经典控件
